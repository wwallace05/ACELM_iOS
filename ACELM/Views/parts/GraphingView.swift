...
struct GraphingView: View {
    ...
    @State var selectedTimeFrame = viewTimeFrame.Minute
    ...
    var body: some View {
        ...
        //let graph1 = cGraph(OutletX: $Outlet1, selectedTimeFrame: $selectedTimeFrame, selectedGraphType: $selectedGraphType, rate: $rate)
        let graph2 = cGraph(OutletX: $Outlet2, selectedTimeFrame: $selectedTimeFrame, selectedGraphType: $selectedGraphType, rate: $rate)
        let graph3 = cGraph(OutletX: $Outlet3, selectedTimeFrame: $selectedTimeFrame, selectedGraphType: $selectedGraphType, rate: $rate)
...
func selectDataToGraph(outlet: Outlet,timeFrame: viewTimeFrame) -> [powerDataPointSummary]{
    ...
    if (timeFrame == viewTimeFrame.Hour) {
        print(">>>> Elements in \(outlet.name) hour power stream [\(outlet.hourPowerStream.count)]")
